    ...
    <div className="bg-brand-bgCard rounded-2xl border border-brand-border overflow-hidden shadow-brand flex flex-col transition-all duration-200 hover:-translate-y-1 hover:shadow-brand-md cursor-pointer">
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="mt-auto flex items-center justify-between">
          <div>
            <p className="text-[11px] text-brand-slate font-semibold uppercase">Price</p>
            <p className="text-[18px] font-extrabold text-brand-tealDark">
              {formatCurrency(property.price)}
            </p>
          </div>
          <button className="px-4 py-2.5 rounded-lg bg-brand-tealBg text-brand-tealDark border border-brand-tealBorder text-xs font-semibold cursor-pointer transition-all duration-200 hover:bg-brand-teal hover:text-white">
            View Details
          </button>
        </div>
      </div>
